...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, hparams: Namespace) -> None:
        ...
        self.precision = MulticlassPrecision(num_classes=8, average='macro')
        self.recall = MulticlassRecall(num_classes=8, average='macro')
    ...
    def shared_val_step(self, batch, batch_idx, prefix):
        ...
        self.precision(labels_hat, y)
        self.recall(labels_hat, y)

        self.log(f"{prefix}_acc", self.accuracy, on_epoch=True)
        self.log(f"{prefix}_f1macro", self.f1macro, on_epoch=True)
        self.log(f"{prefix}_f1weighted", self.f1weighted, on_epoch=True)
        self.log(f"{prefix}_precision", self.precision, on_epoch=True)
        self.log(f"{prefix}_recall", self.recall, on_epoch=True)
        self.log(f"{prefix}_loss", val_loss, on_epoch=True)
