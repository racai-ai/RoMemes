...
OUTPUT_DIR="ocr_results/tessdata_filters-90"
...
for IMAGE in "$INPUT_DIR"/*; do
    ...
    # 5. apply threshold
    magick "$output_image" -threshold 90% "$output_image"
    ...
    tesseract "$output_image" "$OUTPUT_DIR/$BASENAME" -l ron --psm 12 --tessdata-dir /opt/homebrew/share/tessdata
done
